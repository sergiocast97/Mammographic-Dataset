...
\hlcom{# Creating a vector that will store the predictions}
\hlstd{predictedValues} \hlkwb{<-} \hlkwd{as.integer}\hlstd{(regressionModel}\hlopt{$}\hlstd{fitted.values} \hlopt{>} \hlnum{0.5}\hlstd{)}

\hlcom{# Testing predictions against the severity}
\hlstd{error} \hlkwb{<-} \hlkwd{mean}\hlstd{(predictedValues} \hlopt{!=} \hlstd{train}\hlopt{$}\hlstd{Severity)}

\hlcom{# Outputting the overall accuracy of the model}
\hlstd{( training_accuracy} \hlkwb{<-} \hlstd{(}\hlnum{1} \hlopt{-} \hlstd{error)} \hlopt{*} \hlnum{100}\hlstd{)}
\end{alltt}
\begin{verbatim}
## [1] 81.7094
\end{verbatim}
...
\hlstd{probabilities} \hlkwb{<-} \hlkwd{predict}\hlstd{(}
  \hlstd{regressionModel,}                              \hlcom{# Used model}
  \hlkwc{newdata}\hlstd{=}\hlkwd{subset}\hlstd{(validation,}\hlkwc{select}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,}\hlnum{2}\hlstd{,}\hlnum{3}\hlstd{,}\hlnum{4}\hlstd{)),} \hlcom{# Test dataset}
...
\hlstd{( test_accuracy} \hlkwb{<-} \hlkwd{mean}\hlstd{(predictions}\hlopt{==}\hlstd{validation}\hlopt{$}\hlstd{Severity) )}
\end{alltt}
\begin{verbatim}
## [1] 0.812749
\end{verbatim}
...
\begin{alltt}
\hlcom{# ROC Model}
\hlstd{pr} \hlkwb{<-} \hlkwd{prediction}\hlstd{(probabilities, validation}\hlopt{$}\hlstd{Severity)}
...
\end{alltt}
\end{kframe}
\includegraphics[width=.5\linewidth]{figure/unnamed-chunk-28-1} 
...
\hlstd{new_df} \hlkwb{<-} \hlkwd{mice}\hlstd{(}
  ...
  \hlkwc{maxit}\hlstd{=}\hlnum{10}\hlstd{,}         \hlcom{# Number of Iterations}
  \hlkwc{method}\hlstd{=}\hlstr{'polyreg'}\hlstd{,} \hlcom{# Method of imputation (Polyreg is used for categorical values)}
  \hlkwc{seed}\hlstd{=}\hlnum{500}          \hlcom{# Seed number}
\hlstd{)}
\end{alltt}
\begin{verbatim}
## 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{# Create a data partition for Cross Validation}
\hlstd{inTrain} \hlkwb{<-} \hlkwd{createDataPartition}\hlstd{(}
  ...
  \hlkwc{p}\hlstd{=}\hlnum{.7}\hlstd{,}              \hlcom{# Assign 70% of the data to training}
...
\end{verbatim}
\begin{alltt}
\hlkwa{if} \hlstd{(} \hlkwd{nrow}\hlstd{(new_training)} \hlopt{+} \hlkwd{nrow}\hlstd{(new_validating)} \hlopt{==} \hlkwd{nrow}\hlstd{(new_df) )\{}
  \hlkwd{print}\hlstd{(}\hlstr{"The dataset has been split properly"}\hlstd{)}
\hlstd{\}} \hlkwa{else} \hlstd{\{}
  \hlkwd{print}\hlstd{(}\hlstr{"The split of the dataset was not correct"}\hlstd{)}
\hlstd{\}}
